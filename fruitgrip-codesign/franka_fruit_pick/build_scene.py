    ...
    *,
    ...
    for name, layout in YCB_LAYOUT.items():
        ...
        ycb_entities[name] = scene.add_entity(
            ...
            surface=surface,
            # Demo-video asset only (session 4 P4): draws contact-force arrows in
            # the viewer/rendered frames at this object's contact points. Off by
            # default -- irrelevant overhead for search/eval runs.
            visualize_contact=visualize_contacts,
        )

    franka = scene.add_entity(
        gs.morphs.MJCF(
            # Allows swapping in a generated end-effector variant (see
            # franka_fruit_pick/codesign/gripper_gen.py) without touching anything
            # else in the scene; defaults to the stock Franka hand.
            file=franka_xml_path or str(assets / "robots" / "franka" / "panda.xml"),
            pos=FRANKA_POS,
            euler=FRANKA_EULER,
        ),
        visualize_contact=visualize_contacts,
...
def _resize_finger_tail(values: tuple[float, ...], n_finger_dofs: int) -> np.ndarray:
    """Arm portion (first 7) unchanged; finger portion resized to `n_finger_dofs` by
    repeating the last (finger) value -- so a generated N-finger gripper (N != 2, see
    codesign/gripper_gen.py) gets the same per-finger gain/limit as the stock design
    instead of crashing on a shape mismatch.
    """
    arm, fingers = np.array(values[:7]), np.array(values[7:])
    return np.concatenate([arm, np.full(n_finger_dofs, fingers[-1])])


def configure_franka(franka: gs.RigidEntity, *, n_envs: int) -> None:
    n_finger_dofs = franka.n_dofs - 7
    qpos = _resize_finger_tail(FRANKA_QPOS, n_finger_dofs)
    if n_envs > 1:
        qpos = np.tile(qpos, (n_envs, 1))
    franka.set_qpos(qpos)
    franka.set_dofs_kp(_resize_finger_tail(FRANKA_KP, n_finger_dofs))
    franka.set_dofs_kv(_resize_finger_tail(FRANKA_KV, n_finger_dofs))
    franka.set_dofs_force_range(
        _resize_finger_tail(FRANKA_FORCE_MIN, n_finger_dofs), _resize_finger_tail(FRANKA_FORCE_MAX, n_finger_dofs)
    )

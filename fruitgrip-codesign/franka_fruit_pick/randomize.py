...
class EnvRandomizer:
    ...
    def __init__(self, bundle, config: RandomizationConfig | None = None):
        ...
        # Diagnostic-only: the friction ratio _randomize_dynamics already samples every
        # reset(), exposed here (not otherwise readable post-hoc -- RigidEntity has no
        # friction-ratio getter) purely for external logging. Does not affect what's
        # sampled, when, or how -- read-only recording of an existing computation.
    ...
    def _franka_hold_qpos(self) -> np.ndarray:
        """`FRANKA_QPOS` with its finger tail resized to the built franka's actual
        finger-DOF count (repeating the stock finger value) -- so a generated
        N-finger gripper (see codesign/gripper_gen.py) doesn't crash a shape
        mismatch here the same way it originally did in build_scene.configure_franka.
        """
    ...
    def _reset_robot(self) -> None:
        self.bundle.franka.set_qpos(self._franka_hold_qpos(), zero_velocity=True)
    ...
    def _randomize_dynamics(self) -> None:
        ...
        self.last_friction_ratio = ratio  # diagnostic-only recording, see __init__
